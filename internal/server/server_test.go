...
func TestHTTPDomainRedirectsToHTTPS(t *testing.T) {
	cfg := filepath.Join(t.TempDir(), "Runtime")
	runtime := `
example.com {
...
`
	...
	if got := rr.Header().Get("Location"); got != "https://example.com/path?q=1" {
		t.Fatalf("wrong redirect location: %q", got)
	}
}

func TestHTTPIPDoesNotRedirectToHTTPS(t *testing.T) {
	s := New("")
	s.listenAddr = ":80"
	...
	if rr.Code == http.StatusMovedPermanently {
		t.Fatal("expected IP request not to redirect")
	}
}
